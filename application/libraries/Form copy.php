...
class El {
	...
	function __construct($info, $config=array()) 
	{
		// set config vars
...
class Form {
	...
	function config($index=NULL, $init=FALSE)
	{
		...
		$cak = array_keys($config);
		$is_indexed = (is_numeric(array_shift($cak))) ? TRUE : FALSE;
	...
	function captcha($label='')
	{
		...
		$this->text('captcha', $label, 'required|max_length[128]', '', array());
	...
	function recaptcha($label='')
	{
		...
		$info['rules'] = 'required|max_length[128]';
		$this->add($info);
		return $this;
	}
